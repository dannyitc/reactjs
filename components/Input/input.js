...
class Input extends Component {
    ...
    get showLable() {
        const { classes, requiredSymbol, label, labelText } = this.props;
        let disableLabel = false;
        disableLabel = this.props.disableLabel; 
        if(disableLabel){
            return '';
        }
        return  <span className={classes.label}>{requiredSymbol}&nbsp;{labelText}</span>;

    }
    get showIcon(){
        let disableIcon = this.props.disableIcon;
        const { classes } = this.props;
        if(disableIcon == 'true'){
            return '';   
        };
            
        return <span className={classes.iconLetter}></span>;

    }
    render() {
        const { helpText, labelText, requiredSymbol, rootClass, showLable, showIcon } = this;
        const {
            classes,
            placeholder,
            type,
