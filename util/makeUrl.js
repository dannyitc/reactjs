...
const mediaBases = new Map()
    ...
    .set(
        'image-banner',
        process.env.MAGENTO_BACKEND_MEDIA_PATH_BANNER ||
            '/media/tigren/banners'
    );
...
const makeOptimizedUrl = (path, { type, width } = {}) => {
    ...
    if (urlObject.origin != location.origin && (urlObject.origin + '/') != process.env.MAGENTO_BACKEND_URL) return path; 
